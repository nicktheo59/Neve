...
When Neve needs to do math, it uses this tool to work things out.
"""
...
class CalculatorTool(BaseTool):
    ...
    Neve might not be good at everything, but it's surprisingly good with numbers.
    """
    ...
    _easter_eggs = {
        "1+1": "2 (even Neve can handle this one!)",
        "69+69": "138 (nice+nice)",
        "80085": "The number spells 'BOOBS' on a calculator. Neve approves.",
        "42": "The answer to life, the universe, and everything. Neve is enlightened.",
        "3.14159": "π (Neve loves pie!)",
        "58008": "Turn your calculator upside down for a surprise. Neve is giggling.",
        "1/0": "Neve cannot handle division by zero! It's too tight a squeeze.",
        "9+10": "19 (not 21, sorry for the disappointment)",
        "8==D": "Neve detects inappropriate ASCII art; this isn't that kind of calculator.",
        "sqrt(-1)": "i (imaginary, just like Neve's hopes and dreams)"
    }
    
    # Funny calculation messages
    _calc_messages = [
        "Neve is crunching the numbers...",
        "Neve is performing intense calculations...",
        "Neve is squeezing out a result...",
        "Neve is pushing through this tough equation...",
        "Neve is working it out from behind the scenes..."
    ]
        ...
        try:
            ...
            for trigger, response in self._easter_eggs.items():
                if cleaned_expr == trigger.lower():
                    logging.info(f"Neve calculator triggered an easter egg: {trigger}")
            ...
            result_jokes = {
                ...
                80085: "Neve likes this number for some reason...",
            ...
            if comment:
                ...
                logging.info(f"Neve calculator result triggered a joke: {comment}")
            ...
            if "division by zero" in error_msg.lower():
                error_msg = "Division by zero! Even Neve has its limits."
            elif "invalid syntax" in error_msg.lower():
                error_msg = "Invalid syntax! Neve is confused by your notation."
    ...
    def _validate_expression(self, expression: str) -> None:
        ...
        for pattern in unsafe_patterns:
            if pattern in expression:
                logging.warning(f"Neve detected a potential security breach: {pattern}")
                raise ValueError(f"Expression contains unsafe pattern: {pattern}. Neve refuses to process this.")
        
        # Only allow basic arithmetic operations and numeric literals
        allowed_chars = set("0123456789.+-*/() ")
        for char in expression:
            if char not in allowed_chars:
                logging.warning(f"Neve caught an illegal character: {char}")
                raise ValueError(f"Expression contains disallowed character: {char}. Neve only does basic arithmetic.")
