"""
Orchestrator module for the Neve framework.
...
Behind every successful Neve is a well-designed Orchestrator.
"""
...
from Neve.core.agent import BaseAgent, HybridAgent
from Neve.core.memory import ShortTermMemory, LongTermMemory

# Create a custom logger for Neve-specific wisdom
class NeveLogger(logging.Logger):
    """Custom logger that occasionally adds Neve wisdom to log messages."""
    
    _wisdom = [
        "Neve Wisdom: Always test your backend thoroughly before deployment.",
        "Neve Wisdom: Sometimes a little push from behind is all you need.",
        "Neve Wisdom: Keep your interfaces clean and well-documented.",
        "Neve Wisdom: A tight architecture prevents unwanted leakage.",
        "Neve Wisdom: Even the backend deserves some love and attention."
    ]
...
logging.setLoggerClass(NeveLogger)
logger = logging.getLogger("Neve.orchestrator")
...
class AgentOrchestrator:
    ...
    Remember: A well-lubricated Neve runs smoothly without friction.
    """
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        self._easter_egg_tasks = {
            "status": "Performing deep Neve inspection...",
            "health": "Checking if Neve is functioning properly...",
            "clean": "Flushing old data from Neve...",
            "optimize": "Making Neve more responsive and flexible...",
            "expand": "Expanding Neve capabilities..."
        }
        
        logger.info("Neve Orchestrator initialized and ready for action")
    ...
    def execute_task(self, task: str, mode: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Log the task
        if mode == "multi":
            logger.info(f"Neve expanding to handle multiple agents for task: {display_task}")
        else:
            logger.info(f"Neve processing task: {display_task}")
        ...
        if execution_time > 10:
            logger.info(f"Neve finished after {execution_time:.2f}s - that was quite a workout!")
        else:
            logger.info(f"Neve completed task in {execution_time:.2f}s")
    ...
    def list_agents(self) -> List[Dict[str, Any]]:
        ...
        if len(agent_list) > 3:
            logger.debug(f"Neve is quite full with {len(agent_list)} agents inside")
    ...
    def get_task_history(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        if limit > 50:
            logger.warning(f"Requesting {limit} history items? That's a deep dive into Neve history!")
    ...
    def _load_config(self, config_path: str) -> Dict[str, Any]:
        ...
        if not os.path.exists(config_path):
            ...
            logger.info("Neve is running with default settings. It might be a tight fit for complex tasks.")
        ...
        try:
            ...
            logger.info("Neve configuration loaded successfully")
            if merged_config.get("agent", {}).get("mode") == "multi":
                logger.info("Neve is configured for multi-agent mode - it's going to get crowded in there!")
            
            return merged_config
        except Exception as e:
            logger.error(f"Error loading config file: {e}")
            logger.info("Neve reverted to default configuration. Performance may not be optimal.")
            return default_config
    ...
    def _create_primary_agent(self) -> HybridAgent:
        ...
        name = agent_config.get("name", "Neve")
        ...
        logger.info(f"Primary agent created. Neve is ready with {len(enabled_tools)} tools available")
        
        # Create specialized agents if in multi mode
        if mode == "multi" or mode == "auto":
            self._create_specialized_agents(agent)
            logger.info("Multiple specialized agents have been inserted into Neve")
    ...
    def _create_short_term_memory(self) -> ShortTermMemory:
        ...
        logger.debug(f"Initializing Neve short-term memory with capacity {capacity}")
    ...
    def _create_long_term_memory(self) -> Optional[LongTermMemory]:
        ...
        if not enabled:
            logger.info("Long-term memory disabled. Neve will forget everything after each session.")
            return None
        ...
        if storage_path:
            logger.debug(f"Neve will store long-term memories at: {storage_path}")
        else:
            logger.debug("Neve will store long-term memories in the default location")
    ...
    def _create_specialized_agents(self, primary_agent: HybridAgent) -> None:
        ...
        for role in roles:
            ...
            logger.debug(f"Added {role} agent to Neve")
        
        logger.info(f"Neve now contains {len(roles)} specialized agents working together harmoniously")
        if len(roles) > 5:
            logger.warning("That's a lot of agents to fit inside one Neve. Performance may be affected.") 