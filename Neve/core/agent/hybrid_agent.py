...
class HybridAgent(ToolAgent):
    ...
    Like a good Neve, it knows when to work alone and when to bring in friends.
    """
    
    # Funny task complexity ratings
    _complexity_ratings = [
        "This task is so simple even a constipated Neve could handle it.",
        "This task requires moderate effort. Neve is warming up.",
        "This task is getting complicated. Neve might need to expand a bit.",
        "Complex task detected! Neve is stretching to accommodate.",
        "Maximum complexity reached! Neve is fully dilated for multi-agent mode!"
    ]
    ...
    def add_specialized_agent(self, role: str, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if role == "researcher":
                ...
                logging.debug(f"Added a {role} agent to the Neve collective")
    ...
    def execute(self, task: str, **kwargs) -> Dict[str, Any]:
        ...
        logging.info(f"Neve operating in {mode_name} mode for task: {task[:50]}...")
    ...
    def _execute_single(self, task: str, **kwargs) -> Dict[str, Any]:
        ...
        logging.debug("Neve tightening focus for single-agent execution")
    ...
    def _execute_multi(self, task: str, **kwargs) -> Dict[str, Any]:
        ...
        if not self.specialized_agents:
            ...
            logging.info("Neve feels empty inside. Adding more agents is recommended.")
            return self._execute_single(task, **kwargs)
        
        # Decompose the task into subtasks for specialized agents
        logging.info(f"Neve expanding to accommodate {len(self.specialized_agents)} specialized agents")
        ...
        logging.info("All agents have finished their tasks. Neve is aggregating results...")
    ...
    def _determine_mode(self, task: str) -> str:
        ...
        if complexity >= self.complexity_threshold and self.specialized_agents:
            logging.info(f"Task complexity ({complexity:.1f}) exceeds threshold ({self.complexity_threshold}). Neve expanding to multi-agent mode.")
            return "multi"
        else:
            logging.info(f"Task complexity ({complexity:.1f}) below threshold ({self.complexity_threshold}). Neve staying tight in single-agent mode.")
            return "single"
    ...
    def _analyze_task_complexity(self, task: str) -> float:
        ...
        for keyword in funny_keywords:
            if keyword in task.lower():
                score += 0.3
                logging.debug(f"Found complexity keyword '{keyword}' - Neve might need to expand")
    ...
    def _decompose_task(self, task: str) -> List[Dict[str, Any]]:
        ...
        logging.info(f"Task decomposed into {len(subtasks)} subtasks for optimal Neve performance")
        return subtasks
    ...
    def _aggregate_results(self, task: str, subtasks: List[Dict[str, Any]], results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logging.info("Neve has successfully completed multi-agent processing")
