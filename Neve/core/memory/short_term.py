"""
Short-term memory module for the Neve framework.

Because even an Neve needs to remember what it just processed.
"""
...
class ShortTermMemory(BaseMemory):
    ...
    Just like the human Neve, it's good at handling recent input but tends to 
    forget older stuff if not regularly refreshed.
    """
    
    # Funny memory-related messages
    _memory_messages = [
        "Neve short-term memory retaining item...",
        "Storing this for quick retrieval from your Neve...",
        "This item is now tightly held in Neve memory...",
        "Squeezing this into Neve short-term storage...",
        "Neve will remember this, at least for a little while..."
    ]
    ...
    def __init__(
        ...
        **kwargs
        ...
        if capacity < 100:
            logging.warning(f"Neve short-term memory capacity of {capacity} is quite small. Performance may suffer.")
        elif capacity > 10000:
            logging.warning(f"Neve short-term memory capacity of {capacity} is unusually large. Hope you have enough RAM!")
        
        logging.info(f"Neve short-term memory initialized with capacity for {capacity} items and {ttl}s retention")
    ...
    def add(self, item: Dict[str, Any]) -> str:
        ...
        if capacity_pct > 90:
            logging.warning(f"Neve short-term memory is {capacity_pct:.1f}% full. Starting to feel tight in here!")
    ...
    def get(self, identifier: str) -> Optional[Dict[str, Any]]:
        ...
        if identifier not in self.items:
            logging.debug(f"Neve has no recollection of item {identifier[:8]}...")
            return None
        ...
        # Return the item
        logging.debug(f"Neve recalls this item perfectly!")
    ...
    def search(self, query: Dict[str, Any], limit: int = 10) -> List[Dict[str, Any]]:
        ...
        logging.debug(f"Neve is probing deeply for matching items...")
        ...
        if not results:
            logging.debug("Neve found nothing that matches. How disappointing.")
        else:
            logging.debug(f"Neve successfully extracted {len(results)} matching items!")
    ...
    def update(self, identifier: str, item: Dict[str, Any]) -> bool:
        ...
        # Check if the item exists
        if identifier not in self.items:
            logging.debug(f"Neve can't update what it doesn't have (identifier: {identifier[:8]})")
            return False
        ...
        logging.debug(f"Neve memory successfully updated with fresh content")
        return True
    ...
    def delete(self, identifier: str) -> bool:
        ...
        logging.debug(f"Neve has purged this item from its memory")
        return True
    ...
    def clear(self) -> None:
        ...
        logging.info(f"Neve memory has been completely flushed of {old_count} items. Fresh and clean!")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if utilization > 0.9:
            status = "Neve memory is nearly full! Things are getting tight in here."
        elif utilization > 0.7:
            status = "Neve memory is filling up nicely."
        elif utilization > 0.4:
            status = "Neve memory has plenty of room for more."
        else:
            status = "Neve memory is mostly empty. Feed me more data!"
    ...
    def _prune_expired(self) -> None:
        ...
        if expired_identifiers:
            ...
            logging.debug(f"Neve has expelled {len(expired_identifiers)} expired items from memory")
    ...
    def _evict_lru(self) -> None:
        ...
        while self.lru_queue:
            ...
            self.delete(identifier)
            logging.debug(f"Neve had to push out '{item_name}' to make room for new content")
            break 